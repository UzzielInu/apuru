  ...
  <div class="card text-center">
    ...
    <div class="card-body">
      <div class="form-group row justify-content-center my-3">
        ...
        <div class="col-sm-10">
          <input type="text" id="nombre" name="nombre" value="{{$householder->nombre}}" class="form-control text-center">
        </div>
      </div>
      <div class="form-group row justify-content-center my-3">
        <label for="paterno" class="col-sm-2 col-form-label">Apellido Paterno</label>
        <div class="col-sm-10">
          <input type="text" id="paterno" name="paterno" value="{{$householder->paterno}}" class="form-control text-center">
        </div>
      </div>
      <div class="form-group row justify-content-center my-3">
        <label for="materno" class="col-sm-2 col-form-label">Apellido Materno</label>
        <div class="col-sm-10">
          <input type="text" id="materno" name="materno" value="{{$householder->materno}}" class="form-control text-center">
        </div>
      </div>
      <div class="form-group row justify-content-center my-3">
        <label for="extension" class="col-sm-2 col-form-label">Extensión</label>
        <div class="col-sm-10">
          <input type="text" id="extension" name="extension" value="{{$householder->extension}}" class="form-control text-center">
        </div>
      </div>
      <div class="form-group row justify-content-center my-3">
        <label for="correo" class="col-sm-2 col-form-label">Correo</label>
        <div class="col-sm-10">
          <input type="email" id="correo" name="correo" value="{{$householder->correo}}" class="form-control text-center">
        </div>
      </div>
      <div class="form-group row justify-content-center my-3">
        ...
        <div class="col-sm-10  text-center">
          <select id="location_id" class="form-control col-sm-12" name="location_id" style="width: 100%">
            ...
            @foreach ($locations as $item)
              <option value="{{$item->id}}" @if ($item->id == $householder->location->id) selected @endif>{{$item->clave}} -> {{$item->departamento}} [{{$item->areaTrabajo}}]</option>
            @endforeach
          </select>
        </div>
      </div>
